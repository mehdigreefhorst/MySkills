...
| [typed-pydantic-python](skills/typed-pydantic-python/SKILL.md) | Writing or refactoring Python (FastAPI services, shared libs) and deciding how to model data, type signatures, or structure modules — Pydantic in/out for every boundary/stored shape, small atomic models composed and shared by base-class inheritance, `Protocol` seams, no `dict.get` on owned data, `pydantic-settings`, and a `mypy --strict` gate. Includes an audit for bringing an existing project up to the convention. |
| [supabase-db-browser](skills/supabase-db-browser/SKILL.md) | Wanting a GUI over an existing Postgres database without adopting Supabase — attaches Supabase Studio's table editor via `studio` + `postgres-meta` only (2 containers, ~350 MiB, works against vanilla Postgres), surfaces a project's migration `.sql` files read-only in the SQL editor, and names which dashboard pages return fabricated data. Includes lighter alternatives (pgweb 9 MiB, Adminer 27 MiB) and a read-only role. |
