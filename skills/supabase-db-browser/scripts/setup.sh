...
Options:
  ...
  --dir PATH          where to write the compose project (default ./supabase-browser)
...
# Unique per database, so several browsers can run side by side. A fixed name
# makes a second deployment silently recreate the first one's containers.
if [ -z "$NAME" ]; then
  NAME="db-browser-$(printf '%s' "$DB_NAME" | tr '[:upper:]' '[:lower:]' | tr -c 'a-z0-9_-' '-' | sed 's/-*$//')"
fi
...
{
cat <<EOF
# Generated by the supabase-db-browser skill. Studio + postgres-meta only:
